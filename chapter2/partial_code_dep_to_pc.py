...
## 从CSV文件加载深度图数据img并显示
img=np.genfromtxt('chapter2/img_dep_640x480.csv', delimiter=',').astype(np.float32)
...
# print(img[0].shape)
depth_img = []
for v in range(CAM_HGT):
    for u in range(CAM_WID):
        # print(u)
        # input()
        Z = img[v][u]
        X = Z*(u-CAM_CX)/CAM_FX
        Y = Z*(v-CAM_CY)/CAM_FY
        depth_img.append([X,Y,Z])
pc = np.array(depth_img)
#####################################
...
# print(pc)
np.savetxt('chapter2/pc.csv', pc, fmt='%.18e', delimiter=',', newline='\n')

## 从CSV文件加载点云并显示
pc=np.genfromtxt('chapter2/pc.csv', delimiter=',').astype(np.float32)
